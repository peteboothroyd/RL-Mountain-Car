%% settings_dp.m
% *Summary:* Script set up the triple-pendulum scenario with three actuators
%
...
%  2  dtheta2        angular velocity of middle pendulum
%  3  dtheta3        angular velocity of outer pendulum
...
mu0 = [0 0 0 pi pi pi]';                     % initial state mean
S0 = diag([0.1 0.1 0.1 0.01 0.01 0.01].^2);  % initial state covariance
N = 100;                                     % no. of controller optimizations
...
policy.fcn = @(policy,m,s)conCat(@congp,@gSat,policy,m,s);% controller 
                                                          % representation
policy.maxU = [3 3 3];                                    % max. amplitude of 
                                                          % torques
...
policy.p.hyp = ...                                        % initialize policy
  repmat(log([1 1 1 0.7 0.7 0.7 0.7 0.7 0.7 1 0.01]'), 1,3);        % hyper-parameters
...
cost.width = 0.75;                          % cost function width
